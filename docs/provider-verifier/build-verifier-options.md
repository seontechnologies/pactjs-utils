...
| `enablePending`             | `boolean` — `false`                        | When `true`, pacts that have never been successfully verified do not fail the build. Use only as a short-lived bridge when a consumer publishes new interactions before the provider is ready. **Never set permanently in a workflow env block** — it silently bypasses verification for every pending interaction from every consumer. Remove once the consumer's interactions are verified. |
