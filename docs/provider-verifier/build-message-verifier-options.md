...
| `consumerBranch`            | `string` — `env.PACT_CONSUMER_BRANCH`                                     | Adds `{ branch: <name> }` to selectors. Requires `consumer` to also be set (throws otherwise). For stable branch names only; use webhook flow for dynamic PR branches. |
...
| `providerVersion`           | `string` — `env.PACT_PROVIDER_VERSION \|\| env.GITHUB_SHA \|\| 'unknown'` | Provider version string.                                                                                                        |
| `providerVersionBranch`     | `string` — `env.PACT_PROVIDER_BRANCH \|\| env.GITHUB_BRANCH \|\| 'main'`  | Branch name.                                                                                                                    |
