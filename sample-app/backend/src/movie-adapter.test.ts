...
} from '../../shared/test-utils/movie-factories'
...
// Create a deep mock of PrismaClient manually (replacing jest-mock-extended mockDeep)
...
vi.mock('@prisma/client', async () => {
  const actualPrisma =
    await vi.importActual<typeof import('@prisma/client')>('@prisma/client') // eslint-disable-line @typescript-eslint/consistent-type-imports
...
describe('MovieAdapter', () => {
  ...
  beforeEach(() => {
    prismaMock = new PrismaClient() as PrismaMock
  ...
  describe('deleteMovieById', () => {
    ...
    it('should call handleError and rethrow unexpected errors in deleteMovieById', async () => {
      ...
      const handleErrorSpy = vi.spyOn(movieAdapter as any, 'handleError')
