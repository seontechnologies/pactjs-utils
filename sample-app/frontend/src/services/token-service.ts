...
const TOKEN_SERVICE_VERBOSE_LOGS =
  import.meta.env.VITE_TOKEN_SERVICE_VERBOSE_LOGS === 'true'

const shouldLogVerbose = !IS_TEST_ENV || TOKEN_SERVICE_VERBOSE_LOGS
...
export class StorageStateTokenService implements TokenService {
  ...
  setToken(state: StorageState): void {
    ...
    try {
      ...
      logWarn('Failed to extract identity from token:', e)
    }
  ...
  getToken(): StorageState {
    if (!this.currentToken) {
      // logWarn('No token available - will need authentication')
    ...
    try {
      ...
      if (this.isRefreshTokenValid(this.currentToken)) {
        ...
        logInfo('JWT expired but refresh token valid - refreshing...')
        ...
        logWarn('Both JWT and refresh token are expired or invalid')
      }
    } catch (error) {
  ...
  async refreshToken(): Promise<StorageState> {
    ...
    if (this.refreshInProgress) {
      logInfo('Reusing in-progress token refresh request')
  ...
  private async executeRefresh(): Promise<StorageState> {
    try {
      ...
      if (!this.currentToken || !this.isRefreshTokenValid(this.currentToken)) {
        logWarn('No valid refresh token available - cannot renew')
        return this.currentToken || { cookies: [], origins: [] }
      }

      logInfo('Attempting to refresh token with /auth/renew endpoint')
      logInfo('Current cookies before refresh:', document.cookie)
      ...
      logInfo('Token refresh successful - updating storage state')
      logInfo('Response headers:', response.headers)
      logInfo('Cookies after refresh response:', document.cookie)
      ...
      logInfo('Cookies after waiting period:', document.cookie)

      // Capture the current cookies from the browser
      const restored = this.restoreFromBrowserCookies()

      if (!restored) {
        logWarn('Failed to restore cookies after token refresh')
        // Try one more time with a delay
        await new Promise((resolve) => setTimeout(resolve, 500))
        const secondAttempt = this.restoreFromBrowserCookies()
        if (secondAttempt) {
          logInfo('Successfully restored cookies on second attempt')
        } else {
          console.error('All attempts to restore cookies failed')
        }
      } else {
        logInfo('Successfully restored cookies after refresh')
      }
  ...
  private validateTokenTimestamp(tokenContent: string): boolean {
    try {
      ...
      if (colonPos > 0) {
        ...
        logDebug('Extracted timestamp from compound token:', timestamp)
      } else {
        // No colon found, assume the entire token is a timestamp
        logDebug('Using entire token as timestamp:', timestamp)
      }

      // Safety check to ensure timestamp is a valid string
      if (!timestamp || typeof timestamp !== 'string') {
        logDebug('Invalid timestamp format')
        return false
      }
      ...
      if (isNaN(tokenDate.getTime())) {
        logDebug('JWT contains invalid date')
        return false
      }
  ...
  isRefreshTokenValid(token: StorageState): boolean {
    ...
    try {
      ...
      if (!cookie) {
        logDebug('No refresh token cookie found')
        return false
      }
      ...
      if (!isValid) {
        logDebug('Refresh token is expired')
      }
  ...
  async validateAuthWithBackend(): Promise<boolean> {
    try {
      logInfo('Validating authentication with backend...')
      ...
      if (response.status === 200 && response.data.authenticated) {
        ...
        logInfo('Authentication validated successfully')
        return true
      }

      logWarn('Authentication validation failed', response.data)
      return false
    } catch (error) {
  ...
  private restoreFromBrowserCookies(): boolean {
    // Pact consumer tests run in a Node runtime (no browser globals).
    // In that context there is no cookie state to restore.
    ...
    try {
      ...
      if (jwtValue) {
        logInfo('Found JWT cookie, adding to storage state', jwtValue)
        ...
        if (!normalizedValue.includes(':')) {
          ...
          logInfo('Added missing identity to token:', userIdentity)
        }
        ...
        if (normalizedValue.startsWith('Bearer ')) {
          ...
          logInfo('Removed incorrect Bearer prefix from cookie token')
        }
      ...
      if (refreshValue) {
        logInfo('Found refresh cookie, adding to storage state')
        cookies.push({
          name: 'app-refresh',
          value: refreshValue,
          domain: window.location.hostname,
      ...
      this.syncCookiesToBrowser()
      logInfo('Synchronized token state back to browser cookies')
